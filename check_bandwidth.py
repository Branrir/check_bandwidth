...
if usage_up_percent > crit and usage_down_percent > crit:
    print ("CRITICAL: Bandwidth usage Upload: {0}%, Download: {1}%;".format(usage_up_percent, usage_down_percent)+
        ...
        " Upload_band={0};{1};{2};{3};{4}".format(usage_up, int(upload_max/100*warn), int(upload_max/100*crit), 0, upload_max ))
    sys.exit(CRITICAL)
if (usage_up_percent > 100) or (usage_down_percent > 100) or (usage_up_percent < 0) or (usage_down_percent < 0):
    print ("CRITICAL: Network unstable usage over 100%, please check your connection (Possible Change in max Values);" +
        "| Download_band={0};{1};{2};{3};{4}".format(100, int(download_max/100*warn), int(download_max/100*crit), 0, download_max ) + 
        " Upload_band={0};{1};{2};{3};{4}".format(100, int(upload_max/100*warn), int(upload_max/100*crit), 0, upload_max ))
else:
